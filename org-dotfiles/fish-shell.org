...
** GVM
   ...
   As fish shell cannot execute bash scripts, we should use a wrapper
   to the =gvm= script.

   #+BEGIN_SRC shell-script :tangle ~/.config/fish/functions/gvm.fish :padline no :mkdirp yes
     function gvm
         bash -c '. ~/.gvm/bin/gvm-init.sh; gvm "$@"' gvm $argv
     end
   #+END_SRC

   To use the versions managed with =gvm=, we should add the binary
   folders to the fish PATH.

   #+BEGIN_SRC shell-script :tangle ~/.config/fish/config.fish
     set gvmPrograms gaiden gradle grails griffon groovy groovyserv lazybones springboot vertx
     for program in $gvmPrograms
         set -x PATH $PATH ~/.gvm/$program/current/bin
     end
   #+END_SRC
